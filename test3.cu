...
{
	assert(gridDim.x * channel_per_block * filter_per_block == filter_cols * partition * count_per_partition * channels_per_partition);
	assert(gridDim.y == filter_rows);
	assert(gridDim.z * batch_per_block == batch);
	...
	const int partition_idx = blockIdx.x / (channel_group_count * filter_group_count * filter_cols);
	const int origin_x = (blockIdx.x % (channel_group_count * filter_group_count * filter_cols)) / (channel_group_count * filter_group_count);
	const int filter_group_idx = (blockIdx.x % (channel_group_count * filter_group_count)) / channel_group_count;
	const int channel_group_idx = blockIdx.x % channel_group_count;
...
int main(int argc, char** argv)
{
	...
	float* w = 0;
	dim3 thread_per_block(64 / 2, 16 / 4);
	dim3 num_blocks(5 * (96 / 2 / 16) * (256 / 2 / 64) * 2, 5, 16);
	cudaMalloc(&w, sizeof(float) * (256 * 96 / 2) * 5 * 5 * 16);
	int shared_memory_size = sizeof(float) * (16 + 64);
	cudaFuncSetCacheConfig(_cwc_kern_convolutional_backward_propagate_coefficient_default<4, 2, 16, 64, 8>, cudaFuncCachePreferShared);
